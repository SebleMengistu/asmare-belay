@echo off
rem ---------------------------------------------------------------
rem Starts the ASMARE dev stack (Laravel API :8000 + Vite :5174).
...
php artisan migrate --force > "%TEMP%\asmare-migrate.log" 2>&1

echo [2/4] Seeding database only if it has no profile yet...
php artisan tinker --execute="if (App\Models\Profile::count() === 0) { Artisan::call('db:seed'); echo 'SEEDED'; } else { echo 'DATA-OK'; }" > "%TEMP%\asmare-seed.log" 2>&1
type "%TEMP%\asmare-seed.log"

echo [3/4] Starting Laravel API on http://127.0.0.1:8000 ...
netstat -ano | findstr ":8000" | findstr /i "LISTENING" >nul 2>&1
if errorlevel 1 (
    start "asmare-api" /min cmd /c "cd /d %~dp0backend && php artisan serve --host=127.0.0.1 --port=8000"
) else (
...
if errorlevel 1 (
    start "asmare-web" /min cmd /c "cd /d %~dp0frontend && npm run dev"
) else (
