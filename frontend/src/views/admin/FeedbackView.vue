<script setup>
import { computed, onMounted, ref } from 'vue'
import http from '../../api/http'
import BasePagination from '../../components/BasePagination.vue'
import { fmtDate } from '../../utils/format'

const loading = ref(true)
const error = ref('')
const items = ref([])
const stats = ref({ total: 0, unread: 0, avg_rating: 0, categories: [] })
const pagination = ref({ current_page: 1, last_page: 1, total: 0 })
const unreadOnly = ref(false)
const page = ref(1)
const expandedId = ref(null)


async function load() {
  loading.value = true
  error.value = ''
  try {
    const params = { page: page.value }
    if (unreadOnly.value) params.unread = 1

    const res = await http.get('/admin/feedback', { params })
    items.value = res.data ?? []
    pagination.value =
      res.meta?.pagination ?? { current_page: 1, last_page: 1, total: items.value.length }

    const statRes = await http.get('/admin/feedback/stats')
    stats.value = statRes.data ?? { total: 0, unread: 0, avg_rating: 0, categories: [] }
  } catch (e) {
    error.value = e.message
  } finally {
    loading.value = false
  }
}

function toggleReadFilter() {
  unreadOnly.value = !unreadOnly.value
  page.value = 1
  load()
}

function toggleExpand(item) {
  expandedId.value = expandedId.value === item.id ? null : item.id
  if (!item.read_at) markRead(item)
}

async function markRead(item) {
  try {
    const res = await http.post(`/admin/feedback/${item.id}/read`)
    Object.assign(item, res.data ?? {})
    stats.value = { ...stats.value, unread: Math.max(0, Number(stats.value.unread) - 1) }
  } catch (e) {
    error.value = e.message
  }
}

async function removeItem(item) {
  if (!window.confirm('Delete this feedback entry?')) return
  try {
    await http.delete(`/admin/feedback/${item.id}`)
    if (expandedId.value === item.id) expandedId.value = null
    await load()
  } catch (e) {
    error.value = e.message
  }
}

function goTo(n) {
  if (n < 1 || n > pagination.value.last_page) return
  page.value = n
  load()
}

const stars = computed(() => (n) => (n ? '★'.repeat(n) + '☆'.repeat(5 - n) : '—'))

onMounted(load)
</script>

<template>
  <div class="space-y-6">
    <header class="flex flex-wrap items-end justify-between gap-3">
      <div>
        <h1 class="text-2xl font-bold tracking-tight text-slate-900">Feedback</h1>
        <p class="text-sm text-slate-500">
          {{ pagination.total }} total · {{ stats.unread }} unread · avg
          {{ stats.avg_rating || 0 }}/5
        </p>
      </div>

      <div class="flex flex-wrap gap-2">
        <span
          v-for="cat in (stats.categories ?? [])"
          :key="cat.category"
          class="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-600"
        >
          {{ cat.category }} · {{ cat.total }}
        </span>
        <button
          type="button"
          class="rounded-md px-4 py-2 text-sm font-semibold"
          :class="
            unreadOnly
              ? 'bg-brand-600 text-white shadow-sm shadow-brand-600/30'
              : 'border border-slate-300 bg-white text-slate-700 hover:bg-slate-100'
          "
          @click="toggleReadFilter"
        >
          {{ unreadOnly ? 'Showing unread only' : 'Show unread only' }}
        </button>
      </div>
    </header>

    <div v-if="error" class="rounded-lg bg-red-50 p-4 text-sm text-red-700">{{ error }}</div>

    <p v-if="loading" class="text-sm text-slate-500">Loading feedback…</p>
    <p v-else-if="!items.length" class="rounded-lg border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500">
      No feedback yet.
    </p>

    <template v-else>
      <ul class="space-y-3">
        <li
          v-for="item in items"
          :key="item.id"
          class="rounded-xl border bg-white shadow-sm"
          :class="item.read_at ? 'border-slate-200' : 'border-brand-300'"
        >
          <button type="button" class="flex w-full items-center gap-3 p-4 text-left" @click="toggleExpand(item)">
            <span :class="item.read_at ? 'text-transparent' : 'font-bold text-brand-600'">●</span>
            <span class="min-w-0 flex-1">
              <span class="font-medium text-slate-900">{{ item.category || 'General' }}</span>
              <span v-if="item.rating" class="ml-2 text-amber-400">{{ stars(item.rating) }}</span>
              <span class="ml-2 text-sm text-slate-400">{{ item.name || 'Anonymous' }}</span>
            </span>
            <time class="whitespace-nowrap text-xs text-slate-400">{{ fmtDate(item.created_at, { datetime: true }) }}</time>
          </button>

          <div v-if="expandedId === item.id" class="space-y-3 border-t border-slate-100 p-4 text-sm">
            <div>
              <dt class="text-xs font-semibold uppercase tracking-wide text-slate-400">Feedback</dt>
              <dd class="mt-0.5 whitespace-pre-line text-slate-800">{{ item.message }}</dd>
            </div>
            <div v-if="item.email || item.rating">
              <dt class="text-xs font-semibold uppercase tracking-wide text-slate-400">Details</dt>
              <dd v-if="item.rating" class="mt-0.5 text-slate-800">Rating: {{ stars(item.rating) }}</dd>
              <dd v-if="item.email" class="mt-0.5 text-slate-800">{{ item.email }}</dd>
            </div>
            <div v-if="item.ip">
              <dt class="text-xs font-semibold uppercase tracking-wide text-slate-400">IP · Device</dt>
              <dd class="mt-0.5 text-xs text-slate-500">{{ item.ip }} — {{ item.device || 'unknown' }}</dd>
            </div>

            <div class="flex gap-3 pt-1">
              <button
                v-if="!item.read_at"
                type="button"
                class="rounded-md border border-slate-300 px-3 py-1.5 text-sm font-medium hover:bg-slate-100"
                @click.stop="markRead(item)"
              >
                Mark as read
              </button>
              <button
                type="button"
                class="rounded-md border border-red-200 px-3 py-1.5 text-sm font-medium text-red-600 hover:bg-red-50"
                @click.stop="removeItem(item)"
              >
                Delete
              </button>
            </div>
          </div>
        </li>
      </ul>

      <BasePagination :current-page="pagination.current_page" :last-page="pagination.last_page" @change="goTo" />
    </template>
  </div>
</template>
